...
export default function ConditionConfigModal({
  ...
  const handleCalculatePreview = async () => {
    try {
      ...
      setPreviewError(error?.message ?? "Failed to calculate audience split.");
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error?.message ?? "Unable to save condition configuration.");
    }
  };
  ...
  return (
    <Modal
      ...
      size="xl"
      className="max-w-[1400px] lg:w-[90vw]"
    >
