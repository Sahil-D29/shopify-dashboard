...
export default function WhatsAppActionModal({
  ...
  const [formErrors, setFormErrors] = useState<Record<StepId, string[]>>({});
  ...
  const handleRateLimitingChange = (field: keyof RateLimitingConfig, value: number) => {
    setConfig(prev => ({
      ...prev,
      rateLimiting: {
        ...prev.rateLimiting,
  ...
  const handleFailureHandlingChange = (
    ...
    value: number | FailureHandlingConfig["fallbackAction"],
  ) => {
    setConfig(prev => ({
      ...prev,
      failureHandling: {
                ...
                {!config.useDynamicMedia ? (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      disabled={config.useDynamicMedia === true}
                    />
