      ...
      try {
        ...
        if (!cancelled) {
          const approved = Array.isArray(payload.templates)
            ? payload.templates.filter((template: unknown): template is { id: string; name: string; category?: string; language?: string; status?: string } => template && typeof template === 'object' && typeof (template as { id?: unknown }).id === 'string' && (template as { status?: unknown }).status === 'APPROVED')
            : [];
          setTemplates(
