        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {!sorted.length ? (
              ...
              sorted.map(metric => (
                <Fragment key={metric.nodeId}>
                  <tr className="border-t border-[#EFE8E1]">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold text-[#5F4B3F]">
                  ...
                  {metric.variantMetrics && metric.variantMetrics.length ? (
                    <tr className="bg-[#FAF9F6]">
                      <td colSpan={6} className="px-6 pb-4 pt-0">
                        <div className="space-y-2">
                          {metric.variantMetrics.map(variant => (
                            <div
                              key={variant.id}
                              className="flex flex-col gap-2 rounded-2xl border border-[#E8E4DE] bg-white/85 p-3 text-xs text-[#4A4139] shadow-sm md:flex-row md:items-center md:justify-between"
                            >
                              <div className="flex flex-1 flex-col gap-1">
                                <span className="text-sm font-semibold text-[#4A4139]">{variant.label}</span>
                                <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-wide text-[#8B7F76]">
                                  {variant.weight != null ? (
                                    <span className="rounded-full bg-[#EFEAF9] px-2 py-1 text-[#6A5C8F]">
                                      Plan {variant.weight.toFixed(0)}%
                                    </span>
                                  ) : null}
                                  <span className="rounded-full bg-[#F6F1EB] px-2 py-1">
                                    {variant.customers.toLocaleString()} visitors
                                  </span>
                                  <span className="rounded-full bg-[#E2F5EA] px-2 py-1 text-[#2F6B3E]">
                              ...
                              <div className="flex flex-1 items-center gap-2">
                                <div className="h-2 flex-1 rounded-full bg-[#F2ECE6]">
                                  <div
                                    className="h-full rounded-full bg-gradient-to-r from-[#B4A0D6] to-[#7C68B4]"
                                    style={{ width: `${Math.min(100, variant.conversionRate)}%` }}
                                  />
                                </div>
                                <span className="text-xs font-semibold text-[#5F4B3F]">
...
function SortButton({
  ...
  return (
    <Button
      ...
      onClick={onClick}
      className={cn(
        "group flex items-center gap-1 px-0 py-0.5 text-[11px] font-semibold uppercase tracking-[0.25em] text-[#A29386]",
