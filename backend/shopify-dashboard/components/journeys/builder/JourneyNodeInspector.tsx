...
export function JourneyNodeInspector({
  ...
  const experimentStatus = experimentConfig?.status?.status ?? (meta.status?.status as string) ?? 'draft';
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-4 py-5 custom-scrollbar">
        <PanelSection title="Basic Details">
          <div className="space-y-2">
            ...
            <Input
              id="nodeLabel"
              value={meta.label ?? node.data.label}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="nodeDescription"
              rows={3}
              value={meta.description ?? node.data.description ?? ''}
