...
const convertCleverTapRuleToUnified = (rule: CleverTapStyleRule): UnifiedTriggerRule => ({
  ...
  timeFrame: rule.timeFrame
    ? {
        period: (rule.timeFrame.period ?? 'last_7_days') as TimeFrame['period'],
        customDays: rule.timeFrame.customDays,
      }
    : undefined,
});
...
const createDefaultCleverTapConfig = (base: UnifiedTriggerConfig): CleverTapStyleTriggerConfig => ({
  ...
  targetSegment: {
    ...
    segmentId: base.targetSegment?.segmentId,
    segmentName: base.targetSegment?.segmentName,
  ...
  estimatedUserCount: base.estimate?.totalAudience ?? undefined,
});
...
const syncUnifiedFields = (config: EnhancedUnifiedTriggerConfig): EnhancedUnifiedTriggerConfig => {
  ...
  const unifiedTarget: UnifiedTriggerConfig['targetSegment'] = {
    ...
    segmentName: targetSegment.segmentName,
...
const findRule = (
  ...
  const group = segment.ruleGroups.find(g => g.id === ref.location.groupId);
...
export function TriggerConfigPanel({
  ...
  useEffect(() => {
    ...
    setSelectedRule(prev => {
      const exists = findRule(enhanced.cleverTapStyle.targetSegment, prev);
  ...
  const handleRuleGroupAdd = () => {
    updateConfig(prev => ({
      ...
      cleverTapStyle: {
        ...prev.cleverTapStyle,
        targetSegment: {
          ...prev.cleverTapStyle.targetSegment,
          ruleGroups: [
  ...
  const handleRuleSelected = (ruleId: string, category: RuleCategory) => {
    ...
    updateConfig(prev => ({
      ...
      cleverTapStyle: {
        ...prev.cleverTapStyle,
        targetSegment: insertRuleAtLocation(prev.cleverTapStyle.targetSegment, pendingLocation, newRule),
      },
    }));
  ...
  const handleRemoveRule = (ruleId: string, location: RuleLocation) => {
    updateConfig(prev => ({
      ...
      cleverTapStyle: {
        ...prev.cleverTapStyle,
        targetSegment: removeRuleAtLocation(prev.cleverTapStyle.targetSegment, location, ruleId),
      },
    }));
  ...
  const updateSelectedRule = (updater: (rule: CleverTapStyleRule) => CleverTapStyleRule) => {
    ...
    updateConfig(prev => ({
      ...
      cleverTapStyle: {
        ...prev.cleverTapStyle,
        targetSegment: updateRuleAtLocation(prev.cleverTapStyle.targetSegment, location, selectedRuleData.rule.id, updater),
      },
    }));
  };
  ...
  const handleRuleSave = (updatedRule: CleverTapStyleRule) => {
    ...
    updateConfig(prev => ({
      ...
      cleverTapStyle: {
        ...prev.cleverTapStyle,
        targetSegment: updateRuleAtLocation(prev.cleverTapStyle.targetSegment, location, rule.id, () => updatedRule),
      },
    }));
