...
function summariseGoalConfig(config: GoalConfig | undefined | null): string {
  ...
  switch (goalType) {
    ...
    case 'segment_entry':
      return `Segment entry • ${config.segmentId || config.segmentName || 'segment'}`;
    case 'shopify_event':
    ...
    default:
      return goalType.toString().replace(/_/g, ' ');
  }
}
...
const buildUnifiedTriggerSummary = (config: UnifiedTriggerConfig): string | undefined => {
  if (!isValidTriggerConfig(config) || config.targetSegment.rules.length === 0) {
  ...
  const segmentName = config.targetSegment?.segmentName ?? 'Trigger configured';
  return segmentName;
};
...
function normaliseTriggerConfiguration(config: JourneyTriggerConfiguration): JourneyTriggerConfiguration {
  ...
  switch (cloned.category) {
    ...
    case 'shopify_event': {
      const shopify = cloned.shopifyEvent ?? {};
      cloned.shopifyEvent = {
...
function ensureTriggerConfiguration(meta: JsonMap, node?: JourneyNode): JourneyTriggerConfiguration {
  ...
  const entryFrequency = normaliseEntryFrequency(meta?.entryFrequency);
  const entryWindow = normaliseEntryWindow(meta?.entryWindow);
  const estimate = meta?.estimate ? deepClone(meta.estimate) : undefined;
  ...
  switch (triggerType) {
    ...
    case 'product_viewed':
    case 'webhook': {
      const nodeShopify = (nodeTrigger?.data?.shopifyEvent as ShopifyEventTriggerConfig | undefined) ?? {};
      ...
      config = {
        ...
        shopifyEvent: {
          eventType,
          productSelection: shopifyMeta?.productSelection ?? nodeShopify?.productSelection ?? meta?.productSelection ?? {
            mode: 'any',
            productIds: [],
            collectionIds: [],
          },
...
function buildMetaFromNode(node: JourneyNode): JsonMap {
  ...
  switch (node.type) {
    ...
    case 'goal': {
      ...
      if (goalConfig) {
        meta.goalConfig = goalConfig;
        meta.goalType = goalConfig.goalType;
        meta.goalName = goalConfig.goalName;
        meta.goalDescription = goalConfig.goalDescription;
        meta.goalCategory = goalConfig.goalCategory;
        meta.attributionWindow = goalConfig.attributionWindow;
        meta.attributionModel = goalConfig.attributionModel;
        meta.exitAfterGoal = goalConfig.exitAfterGoal;
        meta.markAsCompleted = goalConfig.markAsCompleted;
        meta.countMultipleConversions = goalConfig.countMultipleConversions;
        meta.goalSummary = summariseGoalConfig(goalConfig);
      }
      break;
    }
    default:
...
function createJourneyNodeFromFlow(node: Node<JourneyNodeData>): JourneyNode {
  ...
  if (base.type === 'trigger') {
    ...
    base.action = {
      ...
      templateName: meta.templateName || node.data.label || 'WhatsApp Template',
      language: meta.templateLanguage || 'en',
      variables: meta.variables || {},
      fallbackText: meta.fallbackText,
      sendWindow:
        meta.sendWindowStart || meta.sendWindowEnd
          ? {
              startHour: hourStringToNumber(meta.sendWindowStart) ?? 9,
              endHour: hourStringToNumber(meta.sendWindowEnd) ?? 21,
            }
          : undefined,
    };
    ...
    if (node.data.variant === 'experiment') {
      ...
      base.condition = {
        kind: (meta.conditionType as string) || 'custom_condition',
        args: {
    ...
    if (goalConfig) {
      ...
      base.goal = {
        description: meta.goalDescription || meta.goalName || node.data.description,
      };
    }
...
function mapDomainNodeToFlow(node: JourneyNode): Node<JourneyNodeData> {
  const inferredSubtype = (() => {
    ...
    if (node.type === 'delay') {
      return node.delay?.type || 'fixed_delay';
    }
...
function mapDomainEdgeToFlow(edge: JourneyEdge): Edge {
  return {
    id: edge.id,
    source: edge.source,
    target: edge.target,
...
function mapFlowEdgeToDomain(edge: Edge): JourneyEdge {
  return {
    id: edge.id,
    source: edge.source,
    target: edge.target,
...
function JourneyBuilderInner({ journeyId }: JourneyBuilderInnerProps) {
  ...
  const reactFlow = useReactFlow<JourneyNodeData>();
  const [isCanvasReady, setIsCanvasReady] = useState(false);
  const reactFlowInstanceRef = useRef<ReactFlowInstance<JourneyNodeData> | null>(null);
  ...
  const handleReactFlowInit = useCallback(
    (instance?: ReactFlowInstance<JourneyNodeData>) => {
      ...
      const viewport = preferences?.journeyViewports?.[journeyId] as Viewport | undefined;
  ...
  const handleUpdateNodeMeta = useCallback(
    (nodeId: string, meta: JourneyNodeData['meta']) => {
      ...
      } else if (experimentUpdate) {
        const validIds = new Set(experimentUpdate.map(variant => variant.id));
        setEdges(currentEdges =>
          currentEdges
            ...
            .map(edge => {
              ...
              const match = experimentUpdate!.find(variant => variant.id === edge.sourceHandle);
    ...
    (nodeId: string) => {
      setNodes(nds => {
        ...
        if (clonedNode.data.variant === 'trigger' && clonedNode.data.triggerConfig) {
          ...
          clonedNode.data.triggerConfig = safeCloneMeta(clonedNode.data.triggerConfig) as any;
        }
  ...
  const toolbarValidationSummary = useMemo(() => {
    ...
    const status = errorCount > 0 ? 'fail' : warningCount > 0 ? 'needs_attention' : 'pass';
    return {
      status,
      errors: errorCount,
  ...
  const debouncedAutoSave = useMemo(
    () =>
      debounce((payload: JourneySavePayload) => {
        void autoSaveJourney(payload);
      }, 1000),
    [autoSaveJourney]
  );
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const previous = journeyRef.current;
      const nextJourney: JourneyDefinition = {
        ...(previous ?? { id: journeyId, createdAt: updatedAt.toISOString(), stats: previous?.stats }),
        id: journeyId,
        name: payload.name,
        status: payload.status,
        settings: payload.settings,
