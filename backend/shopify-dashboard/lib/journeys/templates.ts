...
export function createJourneyDefinitionFromTemplate(
  ...
  const defaultConfig: JourneyConfig = {
    ...
    maxEnrollments: null,
    timezone: template.journey.settings?.timezone || 'UTC',
  };
  ...
  return {
    ...
    createdAt: now,
    updatedAt: now,
    settings: template.journey.settings,
    config: {
      ...defaultConfig,
      ...(templateConfig || {}),
      reEntryRules: {
  ...
  switch (type) {
    ...
    case 'action': {
      return {
        ...
        action: {
          kind: templateNode.data?.kind || 'whatsapp_template',
          templateName: templateNode.data?.templateName || templateNode.name || 'Message',
          language: templateNode.data?.templateLanguage || 'en',
