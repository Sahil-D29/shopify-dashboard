...
async function executeDelayNode(
  ...
  if (config.delayMode === 'event') {
    ...
    if (config.timeoutDuration && config.timeoutUnit) {
      enrollment.waitingForEventTimeout = new Date(now + convertToMilliseconds(config.timeoutDuration, config.timeoutUnit)).toISOString();
    }
  ...
  if (config.delayMode === 'until' && config.waitUntil) {
    ...
    resumeAt = now + convertToMilliseconds(config.duration, config.unit);
  }
...
function clearFailure(enrollment: JourneyEnrollmentRecord, nodeId: string) {
  if (!enrollment.metadata?.failures?.[nodeId]) return;
  delete enrollment.metadata.failures[nodeId];
  updateEnrollmentRecord(enrollment);
}
...
function pickVariant(
  ...
  const normalised = variants.map(variant => ({
    id: typeof variant.id === 'string' && variant.id.trim().length > 0 ? variant.id : undefined,
