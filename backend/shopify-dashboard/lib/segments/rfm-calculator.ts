...
export function calculateRFM(
  ...
  const lastOrder = customerOrders
    .map(o => new Date(o.created_at || o.updated_at).getTime())
    .sort((a, b) => b - a)[0];
...
export async function calculateAllRFM(
  ...
  customers.forEach(customer => {
    ...
    const customerOrders = orders.filter(
      o => o.customer?.id === customer.id || o.email === customer.email
    );
    ...
    rfmMap.set(String(customer.id), {
      ...
      lastOrderDate: customerOrders.length > 0
        ? new Date(customerOrders[0].created_at || customerOrders[0].updated_at).getTime()
        : undefined,
