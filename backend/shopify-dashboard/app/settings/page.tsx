...
function SettingsContent() {
  ...
  const { currentStore, stores, refreshStores } = useTenant();
...
function TeamAutoAccessSection() {
  ...
  const loadTeam = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({ error: 'Failed to load team' }));
        throw new Error(errorData.error || errorData.message || 'Failed to load team');
      }
      const data = await res.json();
