  ...
  const syncTemplates = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Synced ${result.synced || 0} templates. ${result.statusChanges || 0} status changes.`,
      });
    } catch (error) {
      console.error('[TemplateLibrary] Sync failed', error);
      toast({
        title: 'Sync Failed',
        description: 'Failed to sync templates from Meta',
        variant: 'destructive',
      });
    } finally {
      setSyncing(false);
  ...
  const importFromMeta = async () => {
    try {
      ...
      toast({
        ...
        description: `Imported ${result.imported || 0} templates, updated ${result.updated || 0}`,
      });
    } catch (error) {
      console.error('[TemplateLibrary] Import failed', error);
      toast({
        title: 'Import Failed',
        description: 'Failed to import templates from Meta',
        variant: 'destructive',
      });
    }
  };
