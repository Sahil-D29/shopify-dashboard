...
function SignInForm() {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      // Check settings status before redirecting
      const checkSettingsAndRedirect = async () => {
        try {
          ...
          // 1. Flag exists in localStorage, AND
          // 2. Both configurations are actually configured server-side
          const setupCompleted = isSetupCompletedLocal && 
                                statusData.success && 
                                statusData.status?.settingsCompleted;
          
          if (!setupCompleted) {
            // Settings not completed, redirect to settings
            console.log('[Login] Setup not completed, redirecting to settings');
            router.push('/settings?setup=true');
          } else {
            // Setup completed - go directly to dashboard/home
            console.log('[Login] Setup completed, redirecting to dashboard');
            if (callbackUrl && callbackUrl !== '/' && callbackUrl !== '/dashboard' && !callbackUrl.startsWith('/auth')) {
          ...
          // On error, check localStorage only as fallback
          const isSetupCompletedLocal = StoreConfigManager.isSetupCompleted();
          if (isSetupCompletedLocal) {
            router.push('/');
          } else {
            router.push('/settings?setup=true');
          }
  ...
  const handleCredentialsSignIn = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Check setup completion status and redirect accordingly
        try {
          // First check localStorage for setup completion flag
          const isSetupCompletedLocal = StoreConfigManager.isSetupCompleted();
