...
function MarketingOverview({ data }: { data: MarketingData | null }) {
  if (!data) return null;
  ...
  const channelComparison = [
    {
      channel: 'Email',
      Sent: data.emailSent,
      Engaged: data.emailOpened + data.emailClicked,
    },
    {
      channel: 'WhatsApp',
      Sent: data.whatsappSent,
      Engaged: data.whatsappRead,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-gray-900">Marketing Overview</h2>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          ...
          <p className="text-2xl font-bold text-gray-900">{data.emailSent.toLocaleString()}</p>
        </div>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-2xl font-bold text-gray-900">{data.whatsappSent.toLocaleString()}</p>
        </div>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-2xl font-bold text-green-600">{data.emailOpenRate}%</p>
        </div>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 className="h-5 w-5 text-blue-500" />
            <p className="text-sm text-gray-500">WhatsApp Delivery</p>
          </div>
          <p className="text-2xl font-bold text-blue-600">{data.whatsappDeliveryRate}%</p>
        </div>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          <div className="flex items-center gap-2 mb-2">
            <MousePointerClick className="h-5 w-5 text-purple-500" />
            <p className="text-sm text-gray-500">Combined Engagement</p>
          </div>
          <p className="text-2xl font-bold text-purple-600">
            {(data.emailSent + data.whatsappSent) > 0
              ? (((data.emailOpened + data.emailClicked + data.whatsappRead) / (data.emailSent + data.whatsappSent)) * 100).toFixed(1)
              : '0.0'}%
          </p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center gap-2 border-b border-gray-200 bg-gray-50 px-6 py-4">
          ...
          <h3 className="text-base font-semibold text-gray-900">Channel Comparison</h3>
        </div>
        <div className="p-6">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={channelComparison}>
              ...
              <Bar dataKey="Engaged" fill="#10b981" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
...
function DashboardContent() {
  ...
  const loadDashboardData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        // Helper to extract response from Promise.allSettled result
        const getResponse = (result: PromiseSettledResult<Response>, name: string): Response => {
          if (result.status === 'rejected') {
            throw new Error(`Failed to fetch ${name}: ${result.reason}`);
          }
          if (!result.value.ok) {
            throw new Error(`Failed to load ${name}: ${result.value.status} ${result.value.statusText}`);
          }
          return result.value;
        };
        ...
        try {
          analytics = await parseJsonResponse<ShopifyAnalyticsSummary>(
            getResponse(analyticsRes, 'analytics'),
            'analytics'
          );
        } catch (error) {
        ...
        try {
          ordersData = await parseJsonResponse<ShopifyOrderListResponse>(
            getResponse(ordersRes, 'orders'),
            'orders'
          );
        } catch (error) {
        ...
        try {
          productsData = await parseJsonResponse<ShopifyProductListResponse>(
            getResponse(productsRes, 'products'),
            'products'
          );
        } catch (error) {
        ...
        try {
          customersData = await parseJsonResponse<ShopifyCustomerListResponse>(
            getResponse(customersRes, 'customers'),
            'customers'
          );
        } catch (error) {
        ...
        try {
          locationsData = await parseJsonResponse<ShopifyLocationListResponse>(
            getResponse(locationsRes, 'locations'),
            'locations'
          );
        } catch (error) {
        ...
        try {
          checkoutsData = await parseJsonResponse<ShopifyCheckoutListResponse>(
            getResponse(checkoutsRes, 'checkouts'),
            'checkouts'
          );
        } catch (error) {
  ...
  // Fetch marketing overview (email + whatsapp combined)
  useEffect(() => {
    if (!isMounted) return;
    const fetchMarketing = async () => {
      try {
        const emailApiBase = process.env.NEXT_PUBLIC_EMAIL_API || 'http://localhost:5000/api/email';
        const endDate = new Date().toISOString();
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - 30);

        const [emailRes, campaignRes] = await Promise.allSettled([
          fetch(`${emailApiBase}/analytics/overview?startDate=${startDate.toISOString()}&endDate=${endDate}`),
          fetch('/api/campaigns'),
        ]);

        let emailData = { sent: 0, opened: 0, clicked: 0, openRate: '0.00' };
        if (emailRes.status === 'fulfilled' && emailRes.value.ok) {
          const d = await emailRes.value.json();
          const totals = d.analytics?.totals || {};
          emailData = {
            sent: totals.sent || 0,
            opened: totals.opened || 0,
            clicked: totals.clicked || 0,
            openRate: d.analytics?.openRate || '0.00',
          };
        }

        let whatsappData = { sent: 0, delivered: 0, read: 0 };
        if (campaignRes.status === 'fulfilled' && campaignRes.value.ok) {
          const d = await campaignRes.value.json();
          const campaigns = d.campaigns || [];
          for (const c of campaigns) {
            if (c.channel === 'whatsapp' || !c.channel) {
              const m = c.metrics || {};
              whatsappData.sent += m.sent || 0;
              whatsappData.delivered += m.delivered || 0;
              whatsappData.read += m.read || 0;
            }
          }
        }

        setMarketingData({
          emailSent: emailData.sent,
          emailOpened: emailData.opened,
          emailClicked: emailData.clicked,
          emailOpenRate: emailData.openRate,
          whatsappSent: whatsappData.sent,
          whatsappDelivered: whatsappData.delivered,
          whatsappRead: whatsappData.read,
          whatsappDeliveryRate: whatsappData.sent > 0
            ? ((whatsappData.delivered / whatsappData.sent) * 100).toFixed(1)
            : '0.0',
        });
      } catch (error) {
  ...
  useAutoRefresh(() => {
    loadDashboardData(true);
  }, 30000, true); // Refresh every 30 seconds
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
        ...
        {/* Marketing Overview - Email + WhatsApp */}
