...
export async function GET(_request: NextRequest, { params }: { params: Params | Promise<Params> }) {
  try {
    ...
    // Determine trigger context from journey trigger
    let triggerContext: 'generic' | 'order' | 'product' = 'generic';
    const trigger = journey.trigger;
    if (trigger?.type === 'order_created' || trigger?.type === 'order_updated') {
      triggerContext = 'order';
    } else if (trigger?.type === 'product_viewed' || trigger?.type === 'product_purchased') {
