...
export async function POST(
  ...
  try {
    const segment = SegmentsStore.get(params.id);
    if (!segment) {
      return NextResponse.json({ error: 'Segment not found' }, { status: 404 });
    }

    const cacheKey = buildCacheKey(params.id);
    const cached = cache.get<SegmentPreviewResponse>(cacheKey);
    if (cached) return NextResponse.json(cached);

    const client = getShopifyClient(request);
    const customersData = (await client.getCustomers({ limit: 250 })) as ShopifyCustomerResponse;
