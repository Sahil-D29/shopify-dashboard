...
export async function GET(
  ...
  try {
    ...
    const store = await findStoreById(params.id);
    ...
    const users = await readStoreUsers(params.id);
...
export async function PATCH(
  ...
  try {
    ...
    const updatedStore = await updateStore(params.id, updates);
  ...
  try {
    ...
    await import('@/lib/auth/admin-auth').then(({ logAdminAction }) =>
      logAdminAction(
        adminSession.userId,
        'store_deleted',
        { storeId: params.id, name: store.name },
        request.headers.get('x-forwarded-for') || null,
        'success'
      )
    );
